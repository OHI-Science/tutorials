...
My friend is a dedicated R user but hasn't had exposure to the awesomeness of R Studio and their many contributions to the scientific community. Since she is a new professor starting up her lab, we are starting a series of Skype calls and screen sharing to get her comfortable (and converted!). For our first call, I tried to categorize the awesomeness that would be most accessible for her needs: Intro to the IDE, R Packages, and Support.


## 1. The RStudio IDE.

IDE stands for integrated development environment. I'm not sure if this analogy works, but if R was an airplane, R Studio would be the airport and traffic control.

Here are the main features to look at as we walk through the IDE together. Check out the [webinar](https://www.rstudio.com/resources/webinars/) and [RStudio IDE cheatsheet](https://www.rstudio.com/resources/cheatsheets/) for more.
...
R Studio has made a bunch of packages that are complete game-changers for data wrangling and analysis. Chains (`%>%`) make your code more readable, with fewer temporary variables.
...
- `knitr`, `rmarkdown` make documents in markdown with executable R code, eg: [.rmd](https://github.com/jafflerbach/spatial-analysis-R/blob/master/intro_spatial_data_R.Rmd), [rendered .html](https://cdn.rawgit.com/eco-data-science/spatial-analysis-R/master/intro_spatial_data_R.html), [rendered .pdf](https://github.com/jafflerbach/spatial-analysis-R/blob/master/intro_spatial_data_R.pdf). Also, make [presentations](http://ohi-science.org/tutorials/tbx_intro/#1)! 
...
R Studio really prioritizes communication and so resources available online are incredible. Google your error messages, and you'll find the R Studio team's instruction, tutorials, bug fixes, etc. Along with [stackoverflow.com](stackoverflow.com), keep an eye out for GitHub issues from R Studio: [github.com/rstudio](https://github.com/rstudio).

- **R Studio developers are real people!** They are leaders in the open science community, and are engaged with the discussions going on. You can follow their conversations or join the discussions easily through Twitter, and GitHub. Here are some of the R Studio heroes, with github and twitter handles:
    - Joe Cheng, jcheng5, @jcheng
    - Hadley Wickham, hadley, @hadleywickham, [hadley.nz](http://hadley.nz/)
    - Yihui Xie, yihui, @xieyihui
- [**webinars**](https://www.rstudio.com/resources/webinars/). Garrett Grolemund (@StatGarrett) leads fantastic webinars about R Studio features, packages, and best practices.
- [**cheatsheets**](https://www.rstudio.com/resources/cheatsheets/). Garrett also makes these cheatsheets that are so well-done and super-useful. Print them all. 
...
The greater R Studio and #rstats community is making some incredible resources for teaching all of this stuff. 
